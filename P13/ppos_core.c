//Projeto P13 - Sistemas Operacionais
//Professor - Carlos Maziero
//Desenvolvido por Thauan Tavares - GRR20171591 
//Data de criação: 24/06/2023
//Data de atualização: 27/06/2023
...
void task_exit (int exit_code){
    ...
    while (atual->esperando != NULL) {
        atual->esperando->ec = exit_code;
        task_resume (atual->esperando, &atual->esperando);
    }
...
void task_suspend (task_t **queue){
    ...
    int existe = 0;
    task_t *aux = filaProntos;
        

    for (int i = 0; i < queue_size((queue_t*)filaProntos); i++) {
        if (aux == atual) {
            existe = 1;
        }
        aux = aux->next;
...
void task_resume (task_t *task, task_t **queue){
    ...
    task->status = PRONTA;
    queue_append ((queue_t **) &filaProntos, (queue_t*) task);
    task->prioridadeDinamica = task->prioridadeInicial;
...
void Dispatcher(void *arg){
   ...
   caso pronta nao faz nada por hora, se TERMINADA remove da fila*/
    ...
    while (qntTaskId > 1){
        ...
        //    task_exit(0);
        //}
        tam = queue_size((queue_t*)filaDormindo);
...
/*Funções de Lock para o semaforo*/
...
// Recebe sinal e adiciona na fila de prontos
    if(signum == SIGUSR1){
        disk.sinal = 1;
        queue_append((queue_t**) &filaProntos,(queue_t*) &(disk.tarefaDisk));
    }
}
...
void diskDispatcher(void *arg){
    while(1){
        ...
        //verifica se tem sinal pendente e adiciona tareva suspensa na fila de prontos
        if(disk.sinal == 1){
            disk.sinal = 0;
        ...
        //verifica se o disco esta disponivel para operações e se tem tarefas na fila
        if((disk_cmd(DISK_CMD_STATUS,0,0) == 1) && (disk.filaDisk != NULL)){
            disk.filaSuspensaDisk=disk.filaDisk;
            queue_remove((queue_t**)&disk.filaDisk,(queue_t*)disk.filaDisk);
