...
int sem_down (semaphore_t *s){
    ...
    /*
    atomic_fetch_sub(&s->valor, 1);
    if(atomic_load(&s->valor) < 0){
        atomic_fetch_add(&s->espera, 1);
        task_suspend(&filaProntos);
    }
    return 0;
    */
    /*while(1){
        ...
        while (s->lock==1){
            if(s->valor > 0){
                ...
                //queue_remove((queue_t**)&filaProntos, (queue_t*)atual);
                //atual->status = SUSPENSA;
                //queue_append((queue_t**)&s->next,(queue_t*)atual);
                task_suspend(&filaProntos);
                s->lock=0;
                task_yield();
                return 0;
...
int sem_up (semaphore_t *s){
    ...
    /*atomic_fetch_add(&s->valor, 1);
    if(atomic_load(&s->valor) <= 0){
        atomic_fetch_sub(&s->espera, 1);
        task_suspend(&filaProntos);
    }
    return 0;*/
    /*task_t *aux;
    ...
    while(1){
        ...
        while (s->lock==1){
           ...
           if(s->espera > 0){
                s->espera--;
                //queue_remove((queue_t**)&filaSuspensas, (queue_t*)aux);
                //atual->status = PRONTA;
...
int sem_destroy (semaphore_t *s){
    ...
    //while (atomic_load(&s->espera) > 0) {
    //    atomic_fetch_sub(&s->espera, 1);
    //    task_resume(filaSuspensas,&filaSuspensas);
    //    filaSuspensas=filaSuspensas->next;
    //}
    //
    //return 0;
    /*task_t *aux;
    ...
    while(1){
        s->lock=1;
        while (s->lock==1){
            while (s->espera > 0 && aux->next != filaSuspensas){
                
                s->espera--;
                //queue_remove((queue_t**)&s->next, (queue_t*)atual);
                //atual->status = PRONTA;
                //queue_append((queue_t**)&filaProntos,(queue_t*)atual);       
                task_resume(aux,&filaSuspensas);
                //s->next=s->next->next;
            
                aux=filaSuspensas->next;
            }
            s->lock=0;
            return 0;
        }
