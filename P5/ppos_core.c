...
void inicia_tempo(){
    ...
    int interval_msec = 1;  // Intervalo em milissegundos

    // Configuração do temporizador
    timer.it_value.tv_sec = 0;                    // Valor inicial do temporizador (segundos)
    timer.it_value.tv_usec = interval_msec * 1000; // Valor inicial do temporizador (microssegundos)
    timer.it_interval.tv_sec = 0;                 // Valor de repetição do temporizador (segundos)
    timer.it_interval.tv_usec = interval_msec * 1000; // Valor de repetição do temporizador (microssegundos)
    ...
    if (setitimer(ITIMER_REAL, &timer, NULL) == -1) {
        perror("Erro ao configurar o temporizador");
        exit(1);
    }
}
...
void tratador(int signum){
    if (atual != NULL && atual->id > 1) {
        ...
        // Verifica se o contador de quantum chegou a zero
        if (atual->quantum == 0) {
            // Coloca a tarefa em execução de volta na fila de prontas
            atual->status = PRONTA;
            queue_append((queue_t **)&filaProntos, (queue_t *)atual);

            // Retorna o controle para o dispatcher
            task_switch(&dispatcher);
        }
    }
